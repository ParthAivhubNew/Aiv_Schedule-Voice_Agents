...
export function ScheduleCallModal({ onClose, onCreate, timezone = "Europe/London" }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(9,11,19,0.7)", backdropFilter: "blur(8px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 20 }}>
      <div style={{ width: "100%", maxWidth: 460, background: "rgba(18,22,41,0.95)", borderRadius: 20, border: `1px solid ${C.border}`, padding: 28, boxShadow: "0 24px 60px rgba(0,0,0,0.6)" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
          <div style={{ fontFamily: FONT_DISPLAY, fontWeight: 700, fontSize: 18, color: "#FFFFFF" }}>Schedule Callback</div>
          <button onClick={onClose} style={{ border: "none", background: "none", cursor: "pointer", color: C.slate }}><X size={18} /></button>
        </div>
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div>
            <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "#CBD5E1", marginBottom: 6 }}>Prospect Company</label>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div>
              <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "#CBD5E1", marginBottom: 6 }}>Day</label>
              <input type="text" value={day} onChange={(e) => setDay(e.target.value)} style={{ width: "100%", height: 40, padding: "0 12px", borderRadius: 10 }} />
            </div>
            <div>
              <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "#CBD5E1", marginBottom: 6 }}>Time</label>
              <input type="text" value={time} onChange={(e) => setTime(e.target.value)} style={{ width: "100%", height: 40, padding: "0 12px", borderRadius: 10 }} />
            </div>
          </div>
          <div>
            <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "#CBD5E1", marginBottom: 6 }}>Exact Requested Quote (Optional)</label>
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: 18 }}>
                ...
                >
                  ...
                  <div style={{ fontFamily: FONT_MONO, fontSize: 14.5, color: "#FFFFFF", fontWeight: 700, marginTop: 2 }}>{it.time}</div>
                </div>
                ...
                <div>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    {it.honored && (
                      <span style={{ fontSize: 10.5, fontWeight: 700, color: C.teal, background: C.tealSoft, padding: "2px 7px", borderRadius: 4, border: `1px solid rgba(0,229,195,0.3)` }}>
                        HONORED CALLBACK
                      </span>
                    )}
                  </div>
