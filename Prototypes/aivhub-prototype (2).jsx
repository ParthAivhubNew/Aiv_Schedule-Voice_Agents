...
function NewMissionModal({ onClose, onCreate }) {
  ...
  const updateImportRow = (id, k, v) => setImportRows((r) => r.map((x) => (x.id === id ? { ...x, [k]: v } : x)));
              ...
              <div>
                ...
                {importState === "parsed" && (
                  <div>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6, fontFamily: FONT_BODY, fontSize: 11, fontWeight: 700, color: C.slate, textTransform: "uppercase", letterSpacing: "0.03em" }}>
                    ...
                    <div style={{ maxHeight: 220, overflowY: "auto", border: `1px solid ${C.border}`, borderRadius: 10 }}>
                      {importRows.map((r) => (
                        <div key={r.id} style={{ display: "flex", flexDirection: "column", gap: 6, padding: "8px 10px", borderBottom: `1px solid ${C.border}`, opacity: r.included ? 1 : 0.4 }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                            ...
                            <input
                              ...
                              style={{ flex: 1.3, padding: "5px 8px", borderRadius: 6, border: `1px solid ${C.border}`, fontFamily: FONT_BODY, fontSize: 12 }}
                            />
                            <input
                              value={r.phone}
                              onChange={(e) => updateImportRow(r.id, "phone", e.target.value)}
                              placeholder="Phone"
                              style={{ flex: 1, padding: "5px 8px", borderRadius: 6, border: `1px solid ${C.border}`, fontFamily: FONT_BODY, fontSize: 12 }}
                            />
                            <input
                              value={r.source}
                            ...
                            />
                            {!r.name && (
                              <span title="Missing company name — won't be imported">
...
export default function App() {
  ...
  const createMission = (payload) => {
    ...
    if (payload.tab === "manual" && payload.rows.length) {
      ...
      setNotifications((ns) => [
        { id: "n_" + Date.now(), text: `${payload.rows.length} companies imported — outreach queued`, time: "just now", unread: true, type: "info" },
        ...ns,
      ]);
      openMission(mission);
      return;
