...
const INITIAL_MISSIONS = [
  ...
  { id: "m4", title: "Provided contact list — Q3 warm leads", sector: "Mixed", region: "UK-wide", status: "active", contacted: 5, total: 9, meetingsBooked: 0, created: "26 Aug", source: "manual", prospects: [
    ...
    { id: "p4b", name: "Coventry Precision Ltd", status: "contacted", note: "Answered, said to call back next week", time: "26 Aug" },
              ...
              <div key={c.id} style={{ background: c.booked ? C.greenSoft : C.paperSoft, border: `1px dashed ${c.booked ? C.green : C.border}`, borderRadius: 12, padding: 16, opacity: c.booked ? 1 : 0.7 }}>
                ...
                {c.booked ? (
                  <div style={{ fontFamily: FONT_BODY, fontSize: 12, color: C.green, marginTop: 6, display: "flex", alignItems: "center", gap: 6, fontWeight: 600 }}>
                    <CheckCircle2 size={13} /> Meeting booked — added to Schedule and Meetings
                  </div>
                ) : (
...
function NewMissionModal({ onClose, onCreate }) {
  ...
  const [rows, setRows] = useState([{ id: 1, name: "", phone: "", sourceType: "Website URL", source: "", channel: "auto", fallback: "none" }]);
  ...
  const addRow = () => setRows((r) => [...r, { id: Date.now(), name: "", phone: "", sourceType: "Website URL", source: "", channel: "", fallback: "none" }]);
  ...
  const buildPreviewFromMap = (records, map) => {
    ...
    return validateRows(mapped).map((r) => ({ ...r, included: r.issues.length === 0 }));
  };
  ...
  const updateImportRow = (id, k, v) =>
    setImportRows((r) => validateRows(r.map((x) => (x.id === id ? { ...x, [k]: v } : x))).map((nr, i) => ({ ...nr, included: r[i].included })));
                      ...
                      <div style={{ display: "flex", gap: 6 }}>
                        {[
                          ...
                          <button
                            ...
                            disabled={id !== "all" && !flaggedCount}
                            style={{
                              padding: "4px 9px", borderRadius: 999, border: `1px solid ${importFilter === id ? C.ink : C.border}`,
                              background: importFilter === id ? C.ink : "#fff", color: importFilter === id ? "#fff" : (id !== "all" && !flaggedCount ? C.slateLight : C.slate),
                              fontFamily: FONT_BODY, fontSize: 11, fontWeight: 600, cursor: id !== "all" && !flaggedCount ? "default" : "pointer",
                            }}
                          >
                            {label}
                          </button>
                    ...
                    {flaggedCount > 0 && (
                      <div style={{ display: "flex", alignItems: "center", gap: 8, background: C.amberSoft, borderRadius: 8, padding: "8px 10px", marginBottom: 8 }}>
                        ...
                        <span style={{ fontFamily: FONT_BODY, fontSize: 11.5, color: C.textInk, flex: 1 }}>
                          {flaggedCount} row{flaggedCount === 1 ? "" : "s"} flagged (missing info, bad number, or duplicate) and left unchecked — they won't be contacted unless you fix and re-check them.
                        </span>
                          ...
                          <div style={{ display: "flex", alignItems: "center", gap: 6, paddingLeft: 22, flexWrap: "wrap" }}>
                            ...
                            {r.issues.length > 0 && (
                              <span style={{ display: "flex", gap: 4, marginLeft: "auto", flexWrap: "wrap" }}>
                                {r.issues.map((code) => (
                                  <span key={code} style={{ fontFamily: FONT_BODY, fontSize: 10, fontWeight: 700, color: ISSUE_META[code].color, background: "#fff", border: `1px solid ${ISSUE_META[code].color}`, borderRadius: 999, padding: "1px 6px" }}>
                                    {code === "duplicate" ? `Duplicate of ${r.duplicateOf}` : ISSUE_META[code].label}
                                  </span>
                                ))}
                              </span>
                            )}
...
export default function App() {
  ...
  // this is the wiring the person asked for: when the AI negotiates a time on a live call/
  // message, "confirming" it doesn't just sit in that transcript — it creates a real entry
  // in both Schedule and Meetings, and the call itself flips to a "booked" resolved state.
  const confirmBooking = (call) => {
    const day = "Today, 27 Aug";
    const time = "16:00"; // next plausible slot inside the 09:00–17:30 window, for demo purposes

    setScheduleItems((its) => [
      ...its,
      { id: "s_" + Date.now(), day, time, prospect: call.prospect, mission: call.mission, window: "09:00–17:30", status: "queued" },
    ]);
    ...
    setMeetings((ms) => [
      {
        ...
        prep: "Booked live by the AI during this conversation — no additional prep notes yet.",
    ...
    setNotifications((ns) => [
      { id: "n_" + Date.now(), text: `Meeting booked with ${call.prospect} — added to Schedule and Meetings`, time: "just now", unread: true, type: "success" },
      ...ns,
    ]);
  };
  ...
  const createMission = (payload) => {
    ...
    if (payload.tab === "manual" && payload.rows.length) {
      ...
      const prospects = payload.rows.map((r, i) => {
        const effChannel = r.channel || payload.channel; // per-row override, else mission default
        ...
        // first `concurrency` rows shown as actively dialing now, rest waiting their turn —
        // gives visible signal of what's actually happening on a big batch, not just a total count
        const status = i < concurrency ? "calling" : "queued";
        return {
          id: "up_" + Date.now() + "_" + i,
          name: r.name,
          status,
      ...
      setNotifications((ns) => [
        { id: "n_" + Date.now(), text: `${payload.rows.length} companies imported — ${Math.min(concurrency, payload.rows.length)} calls starting now, rest queued`, time: "just now", unread: true, type: "info" },
        ...ns,
      ]);
      openMission(mission);
      return;
  ...
  return (
    <div style={{ display: "flex", height: "100vh", background: C.paper, fontFamily: FONT_BODY }}>
      ...
      <div style={{ flex: 1, overflowY: "auto" }}>
        ...
        {view === "schedule" && <ScheduleView notifications={notifications} setNotifications={setNotifications} prefillName={prefillSchedule} clearPrefill={() => setPrefillSchedule(null)} items={scheduleItems} setItems={setScheduleItems} />}
